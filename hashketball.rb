...
def num_points_scored (player_name)
  ...
  game_hash[:home][:players].each do |player_data|
    if player_name == player_data[:player_name]
      home_player_points = player_data[:points]
    end
  end
  ...
  game_hash[:away][:players].each do |player_data|
    if player_name == player_data[:player_name]
      away_player_points = player_data[:points]
    end
  end  
  
  return home_player_points + away_player_points
end
...
def shoe_size (player_name)
  ...
  game_hash[:home][:players].each do |player_data|
    if player_name == player_data[:player_name]
      home_player_shoe = player_data[:shoe]
    end
  end
  ...
  game_hash[:away][:players].each do |player_data|
    if player_name == player_data[:player_name]
      away_player_shoe = player_data[:shoe]
    end
  end  
  
  return home_player_shoe + away_player_shoe
end
...
def player_numbers (team_name)
  game_hash.each do |team, team_data|
    if team_data[:team_name] == team_name
      return team_data[:players].map do |player_data|
        player_data[:number]
      end
    end
  end
